/*
 * A LED driver for the accton_wedge100s_32x_led
 *
...
#define DRVNAME "accton_wedge100s_32x_led"
#define ENABLE_PORT_LED		1

struct accton_wedge100s_32x_led_data {
...
static struct accton_wedge100s_32x_led_data  *ledctl = NULL;
...
static void accton_wedge100s_32x_led_set(struct led_classdev *led_cdev,
...
static int accton_wedge100s_32x_led_read_value(u8 reg)
{
	return accton_i2c_cpld_read(LED_CNTRLER_I2C_ADDRESS, reg);
}

static int accton_wedge100s_32x_led_write_value(u8 reg, u8 value)
{
	return accton_i2c_cpld_write(LED_CNTRLER_I2C_ADDRESS, reg, value);
}

static void accton_wedge100s_32x_led_update(void)
{
	...
	if (time_after(jiffies, ledctl->last_updated + HZ + HZ / 2)
		...
		dev_dbg(&ledctl->pdev->dev, "Starting accton_wedge100s_32x_led update\n");

		/* Update LED data
		 */
		for (i = 0; i < ARRAY_SIZE(ledctl->reg_val); i++) {
			int status = accton_wedge100s_32x_led_read_value(led_reg_map[i].reg_addr);
									  ...
									  enum led_type type)
{
	...
	reg_val = accton_wedge100s_32x_led_read_value(reg);
	...
	accton_wedge100s_32x_led_write_value(reg, reg_val);
...
static void accton_wedge100s_32x_led_diag_set(struct led_classdev *led_cdev,
										   enum led_brightness led_light_mode)
{
	accton_wedge100s_32x_led_set(led_cdev, led_light_mode,  LED_TYPE_DIAG);
}

static enum led_brightness accton_wedge100s_32x_led_diag_get(struct led_classdev *cdev)
{
	accton_wedge100s_32x_led_update();
	return led_reg_val_to_light_mode(LED_TYPE_DIAG, ledctl->reg_val[0]);
}

static void accton_wedge100s_32x_led_loc_set(struct led_classdev *led_cdev,
										  enum led_brightness led_light_mode)
{
	accton_wedge100s_32x_led_set(led_cdev, led_light_mode, LED_TYPE_LOC);
}

static enum led_brightness accton_wedge100s_32x_led_loc_get(struct led_classdev *cdev)
{
	accton_wedge100s_32x_led_update();
	return led_reg_val_to_light_mode(LED_TYPE_LOC, ledctl->reg_val[0]);
}

static void accton_wedge100s_32x_led_auto_set(struct led_classdev *led_cdev,
										   enum led_brightness led_light_mode)
{
}

static enum led_brightness accton_wedge100s_32x_led_auto_get(struct led_classdev *cdev)
{
...
static int accton_wedge100s_32x_port_led_read_value(unsigned short cpld_addr, u8 reg)
{
	return accton_i2c_cpld_read(cpld_addr, reg);
}

static int accton_wedge100s_32x_port_led_write_value(unsigned short cpld_addr, u8 reg, u8 value)
{
...
static void accton_wedge100s_32x_port_led_set(struct led_classdev *cdev,
										   enum led_brightness led_light_mode)
{
	unsigned int port, lid;
	unsigned short cpld_addr;
	u8 reg, value;
	sscanf(cdev->name, "accton_wedge100s_32x_led::port%u_led%u", &port, &lid);
	...
	accton_wedge100s_32x_port_led_write_value(cpld_addr, reg, value);
}

static enum led_brightness accton_wedge100s_32x_port_led_get(struct led_classdev *cdev)
{
	unsigned int port, lid;
	unsigned short cpld_addr;
	u8 reg, value;
	...
	cpld_addr = (port < 16) ? 0x64 : 0x62;
	reg       = (0x50 + (port % 16) * 4 + lid);
	value 	  = accton_wedge100s_32x_port_led_read_value(cpld_addr, reg);
...
#define _PORT_LED_CLASSDEV(port, lid)									\
	[LED_TYPE_PORT##port##_LED##lid] = {								\
		.name			 = "accton_wedge100s_32x_led::port"#port"_led"#lid,\
		.default_trigger = "unused",									\
		.brightness_set	 = accton_wedge100s_32x_port_led_set,				\
		.brightness_get	 = accton_wedge100s_32x_port_led_get,				\
...
static struct led_classdev accton_wedge100s_32x_leds[] = {
	[LED_TYPE_DIAG] = {
		.name			 = "accton_wedge100s_32x_led::diag",
		.default_trigger = "unused",
		.brightness_set	 = accton_wedge100s_32x_led_diag_set,
		.brightness_get	 = accton_wedge100s_32x_led_diag_get,
		.flags			 = LED_CORE_SUSPENDRESUME,
		.max_brightness	 = LED_MODE_RED,
	},
	[LED_TYPE_LOC] = {
		.name			 = "accton_wedge100s_32x_led::loc",
		.default_trigger = "unused",
		.brightness_set	 = accton_wedge100s_32x_led_loc_set,
		.brightness_get	 = accton_wedge100s_32x_led_loc_get,
		.flags			 = LED_CORE_SUSPENDRESUME,
		.max_brightness	 = LED_MODE_BLUE,
	},
	[LED_TYPE_FAN] = {
		.name			 = "accton_wedge100s_32x_led::fan",
		.default_trigger = "unused",
		.brightness_set	 = accton_wedge100s_32x_led_auto_set,
		.brightness_get  = accton_wedge100s_32x_led_auto_get,
		.flags			 = LED_CORE_SUSPENDRESUME,
		.max_brightness  = LED_MODE_AUTO,
	},
	[LED_TYPE_PSU1] = {
		.name			 = "accton_wedge100s_32x_led::psu1",
		.default_trigger = "unused",
		.brightness_set	 = accton_wedge100s_32x_led_auto_set,
		.brightness_get  = accton_wedge100s_32x_led_auto_get,
		.flags			 = LED_CORE_SUSPENDRESUME,
	...
	[LED_TYPE_PSU2] = {
		.name			 = "accton_wedge100s_32x_led::psu2",
		.default_trigger = "unused",
		.brightness_set	 = accton_wedge100s_32x_led_auto_set,
		.brightness_get  = accton_wedge100s_32x_led_auto_get,
		.flags			 = LED_CORE_SUSPENDRESUME,
...
static int accton_wedge100s_32x_led_suspend(struct platform_device *dev,
		pm_message_t state)
{
	int i = 0;
	
	for (i = 0; i < ARRAY_SIZE(accton_wedge100s_32x_leds); i++) {
		led_classdev_suspend(&accton_wedge100s_32x_leds[i]);
	}

	return 0;
}

static int accton_wedge100s_32x_led_resume(struct platform_device *dev)
{
	int i = 0;
	
	for (i = 0; i < ARRAY_SIZE(accton_wedge100s_32x_leds); i++) {
		led_classdev_resume(&accton_wedge100s_32x_leds[i]);
	}

	return 0;
}

static int accton_wedge100s_32x_led_probe(struct platform_device *pdev)
{
	int ret, i;

	for (i = 0; i < ARRAY_SIZE(accton_wedge100s_32x_leds); i++) {
		ret = led_classdev_register(&pdev->dev, &accton_wedge100s_32x_leds[i]);
	...
	if (i != ARRAY_SIZE(accton_wedge100s_32x_leds)){
		int j;
		
		/* only unregister the LEDs that were successfully registered */
		for (j = 0; j < i; j++) {
			led_classdev_unregister(&accton_wedge100s_32x_leds[i]);
		}
	}
...
static int accton_wedge100s_32x_led_remove(struct platform_device *pdev)
{
	int i;
...
static struct platform_driver accton_wedge100s_32x_led_driver = {
	.probe	  = accton_wedge100s_32x_led_probe,
	.remove	 = accton_wedge100s_32x_led_remove,
	.suspend	= accton_wedge100s_32x_led_suspend,
	.resume	 = accton_wedge100s_32x_led_resume,
	.driver	 = {
	.name   = DRVNAME,
	.owner  = THIS_MODULE,
	},
...
static int __init accton_wedge100s_32x_led_init(void)
{
	int ret;

	ret = platform_driver_register(&accton_wedge100s_32x_led_driver);
	if (ret < 0) {
		goto exit;
	}

	ledctl = kzalloc(sizeof(struct accton_wedge100s_32x_led_data), GFP_KERNEL);
	if (!ledctl) {
		ret = -ENOMEM;
		platform_driver_unregister(&accton_wedge100s_32x_led_driver);
		goto exit;
	}
...
static void __exit accton_wedge100s_32x_led_exit(void)
{
...
module_init(accton_wedge100s_32x_led_init);
module_exit(accton_wedge100s_32x_led_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("accton_wedge100s_32x_led driver");
MODULE_LICENSE("GPL");

/*
 * A hwmon driver for the Accton wedge100s 32x fan
 *
...
#define DRVNAME "wedge100s_32x_fan"
...
static struct wedge100s_32x_fan_data *wedge100s_32x_fan_update_device(struct device *dev);
...
static struct attribute *wedge100s_32x_fan_attributes[] = {
...
static int wedge100s_32x_fan_read_value(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int wedge100s_32x_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
...
static u8 is_fan_fault(struct wedge100s_32x_fan_data *data, enum fan_id id)
{
    u8 ret = 1;
...
static ssize_t set_enable(struct device *dev, struct device_attribute *da,
                          const char *buf, size_t count)
{
    struct wedge100s_32x_fan_data *data = wedge100s_32x_fan_update_device(dev);
    int error, value;
                              ...
                              const char *buf, size_t count)
{
    ...
    wedge100s_32x_fan_write_value(client, 0x33, 0); /* Disable fan speed watch dog */
    wedge100s_32x_fan_write_value(client, fan_reg[FAN_DUTY_CYCLE_PERCENTAGE], duty_cycle_to_reg_val(value));
    return count;
}
...
static int _find_lm75_device(struct device *dev, void *data)
{
    ...
    struct wedge100s_32x_fan_data *prv = data;
...
static const struct attribute_group wedge100s_32x_fan_group = {
    .attrs = wedge100s_32x_fan_attributes,
};

static struct wedge100s_32x_fan_data *wedge100s_32x_fan_update_device(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct wedge100s_32x_fan_data *data = i2c_get_clientdata(client);
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
        ...
        dev_dbg(&client->dev, "Starting wedge100s_32x_fan update\n");
        data->valid = 0;

        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = wedge100s_32x_fan_read_value(client, fan_reg[i]);
...
static int wedge100s_32x_fan_probe(struct i2c_client *client,
                                const struct i2c_device_id *dev_id)
{
    struct wedge100s_32x_fan_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct wedge100s_32x_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &wedge100s_32x_fan_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &wedge100s_32x_fan_group);
exit_free:
    kfree(data);
exit:
...
static int wedge100s_32x_fan_remove(struct i2c_client *client)
{
    struct wedge100s_32x_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id wedge100s_32x_fan_id[] = {
    { "wedge100s_32x_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, wedge100s_32x_fan_id);

static struct i2c_driver wedge100s_32x_fan_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRVNAME,
    },
    .probe        = wedge100s_32x_fan_probe,
    .remove       = wedge100s_32x_fan_remove,
    .id_table     = wedge100s_32x_fan_id,
    .address_list = normal_i2c,
};

static int __init wedge100s_32x_fan_init(void)
{
    return i2c_add_driver(&wedge100s_32x_fan_driver);
}

static void __exit wedge100s_32x_fan_exit(void)
{
    i2c_del_driver(&wedge100s_32x_fan_driver);
}

module_init(wedge100s_32x_fan_init);
module_exit(wedge100s_32x_fan_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("wedge100s_32x_fan driver");
MODULE_LICENSE("GPL");
